...
export default function UnifiedCreator() {
  ...
  const [imgFiles, setImgFiles] = useState([]) // [{ file, preview, name }]
  const [imgTipo, setImgTipo] = useState('post') // 'post' | 'carrossel'
...
Gere exatamente 5 hooks para o tema dado. Responda EXCLUSIVAMENTE com JSON: {"hooks": ["hook1","hook2","hook3","hook4","hook5"]}`,
  ...
  const handleImgUpload = (e) => {
    ...
    const parsed = files.map(f => {
      ...
      return { file: f, preview, name: f.name }
    })
...
${imgContexto ? `CONTEXTO ADICIONAL DA CRIADORA: ${imgContexto}\n` : ''}

PERSONA: Karen Santos — consultora tech, mentora de carreira, criadora de conteúdo sobre carreira em tecnologia, comportamento profissional, liderança e IA. Tom: direto, analítico, sem floreio, sem motivacional vazio.

${imgTipo === 'carrossel'
  ? `As imagens formam um carrossel. Analise a narrativa visual em sequência e sugira legendas que complementem o conjunto.`
  : `Analise a imagem e sugira legendas que complementem o visual.`
}

REGRAS DE LEGENDA:
- Abertura forte que para o scroll (não genérica)
- Tom autêntico, sem coach speak, sem "incrível", "transformador", "jornada"
- Espaço para a pessoa completar com experiência própria
- CTA natural, não forçado
- 5-8 hashtags relevantes
- NUNCA use: "não é X, é Y" / "o segredo é" / "ninguém fala sobre isso" / anos específicos

Responda EXCLUSIVAMENTE com JSON válido:
{
  "descricao_visual": "o que você vê nas imagens em 1-2 frases",
  "tema_sugerido": "tema central detectado",
  "tom_sugerido": "reflexivo|engracado|mentora",
  "legendas": [
    {
      "versao": "Principal",
      "texto": "legenda completa com abertura + corpo + CTA",
      "hashtags": ["#tag1", "#tag2"]
    },
    {
      "versao": "Mais pessoal",
      "texto": "versão mais íntima e pessoal",
      "hashtags": ["#tag1", "#tag2"]
    },
    {
      "versao": "Mais provocativa",
      "texto": "versão mais direta e incisiva",
      "hashtags": ["#tag1", "#tag2"]
    }
  ]
}`

      const imageBlocks = b64Images.map(img => ({
        type: 'image',
        ...
        <div className="space-y-4 animate-fade-in">
          <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
            ...
            <div>
              ...
              <input
                value={imgContexto}
                onChange={e => setImgContexto(e.target.value)}
                placeholder="Ex: foto do evento X, foto de bastidores, selfie pós-palestra..."
                className="input text-sm w-full"
              />
            </div>

            {imgError && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs text-red-700">{imgError}</div>
            )}

            <button
              onClick={generateCaptionFromImages}
            ...
            <div className="space-y-4 animate-fade-in">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-4">
                ...
                <div className="flex gap-2 mt-2">
                  <span className="text-[10px] font-semibold bg-gray-100 text-gray-500 px-2 py-1 rounded-lg">
                    Tema: {imgResult.tema_sugerido}
                  </span>
                  {imgResult.tom_sugerido && (
                    <span className={clsx('text-[10px] font-semibold px-2 py-1 rounded-lg',
                      ...
                      imgResult.tom_sugerido === 'engracado' ? 'bg-amber-100 text-amber-700' :
                      'bg-blue-100 text-blue-700'
                    )}>
                      Tom: {imgResult.tom_sugerido}
                    </span>
                  )}
                </div>
              </div>
              ...
              {(imgResult.legendas || []).map((leg, i) => (
                <div key={i} className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
                  ...
                  {leg.hashtags?.length > 0 && (
                    <p className="text-xs text-blue-500">{leg.hashtags.join(' ')}</p>
                  )}
                </div>
              ))}
            </div>
