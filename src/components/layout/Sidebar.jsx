...
import {
  ...
  Clapperboard, Sun, Moon, Scissors, Newspaper,
} from 'lucide-react'
  ...
  const handleImport = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data.store) localStorage.setItem(STORE_KEY, JSON.stringify(data.store))
        API_KEYS.forEach(k => {
  ...
  return (
    ...
    >
      <div className="px-5 pt-6 pb-5 border-b border-orange-100 flex items-center justify-between">
        ...
        <button onClick={onClose} className="lg:hidden p-1.5 rounded-lg hover:bg-orange-100 text-gray-400 hover:text-gray-600 transition-colors">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="px-4 pb-5 pt-3 border-t border-orange-100 space-y-3">
        {syncMsg && (
          <div className={`flex items-center gap-2 text-[11px] px-3 py-2 rounded-lg border ${syncMsg.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
            {syncMsg.type === 'success' ? <Check size={12} /> : <AlertCircle size={12} />}
            {syncMsg.text}
          </div>
        )}
        <div className="flex gap-2">
          <button onClick={handleExport} className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-medium text-gray-600 hover:text-orange-700 bg-white hover:bg-orange-50 border border-gray-200 hover:border-orange-200 rounded-lg py-2 transition-all" title="Baixar backup de todos os dados">
            <Download size={12} /> Exportar
          </button>
          <button onClick={() => importRef.current?.click()} className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-medium text-gray-600 hover:text-orange-700 bg-white hover:bg-orange-50 border border-gray-200 hover:border-orange-200 rounded-lg py-2 transition-all" title="Importar backup de outro dispositivo">
            <Upload size={12} /> Importar
          </button>
          <input ref={importRef} type="file" accept=".json" className="hidden" onChange={handleImport} />
        </div>
        <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border border-orange-200 bg-white hover:bg-orange-50 transition-colors text-sm font-medium text-gray-600 hover:text-orange-700">
          {theme === 'dark' ? <><Sun size={15} className="text-orange-500" /> Modo Claro</> : <><Moon size={15} className="text-orange-500" /> Modo Escuro</>}
        </button>
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-orange-600 flex items-center justify-center text-xs font-bold text-white">CU</div>
