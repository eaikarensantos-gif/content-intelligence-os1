...
import {
  ...
  Square, CheckSquare, ClipboardList,
} from 'lucide-react'
...
// ── Inline Task List ───────────────────────────────────────────────────────
function InlineTasks({ brainItemId, tasks, onAdd, onToggle, onDelete }) {
  const [input, setInput] = useState('')
  const itemTasks = tasks.filter((t) => t.brainItemId === brainItemId)
  const done = itemTasks.filter((t) => t.status === 'done').length

  const handleAdd = (e) => {
    e.preventDefault()
    if (!input.trim()) return
    onAdd({ title: input.trim(), brainItemId, status: 'todo', priority: 'medium' })
    setInput('')
  }
  ...
  return (
    <div className="space-y-1.5 pt-2 border-t border-gray-100">
      <div className="flex items-center justify-between">
        <p className="text-[9px] font-semibold text-gray-400 uppercase tracking-wide flex items-center gap-1">
          <ClipboardList size={10} /> Tarefas
        </p>
        {itemTasks.length > 0 && (
          <span className="text-[9px] text-gray-400">{done}/{itemTasks.length}</span>
        )}
      </div>

      {itemTasks.length === 0 && (
        <p className="text-[10px] text-gray-300 italic">Nenhuma tarefa alocada</p>
      )}
      ...
      {itemTasks.map((task) => (
        <div key={task.id} className="flex items-center gap-2 group/task">
          ...
          <span className={`flex-1 text-[11px] leading-tight ${task.status === 'done' ? 'line-through text-gray-300' : 'text-gray-600'}`}>
            {task.title}
          </span>
          <button
            onClick={() => onDelete(task.id)}
            className="opacity-0 group-hover/task:opacity-100 text-gray-200 hover:text-red-400 transition-all shrink-0"
          >
            <X size={11} />
          </button>
        </div>
      ...
      <form onSubmit={handleAdd} className="flex items-center gap-1.5 pt-0.5">
        <input
          ...
          placeholder="+ Alocar tarefa..."
          className="flex-1 text-[11px] border border-gray-200 rounded-md px-2 py-1 outline-none focus:border-orange-300 placeholder-gray-300"
        />
        {input.trim() && (
          <button type="submit" className="text-[10px] bg-orange-500 text-white px-2 py-1 rounded-md hover:bg-orange-600 transition-colors">
            OK
          </button>
        )}
      </form>
...
// ── Item Card ──────────────────────────────────────────────────────────────
function BrainCard({ item, onMove, onDelete, tasks, onAddTask, onToggleTask, onDeleteTask, compact = false }) {
  ...
  const taskCount = tasks.filter((t) => t.brainItemId === item.id).length
  const doneCount = tasks.filter((t) => t.brainItemId === item.id && t.status === 'done').length
    ...
    >
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          {taskCount > 0 && (
            <span className="text-[9px] bg-indigo-100 text-indigo-600 border border-indigo-200 px-1.5 py-0.5 rounded-full font-semibold flex items-center gap-0.5">
              <ClipboardList size={8} /> {doneCount}/{taskCount}
            </span>
          )}
          {expanded
...
export default function ContentBrain() {
  ...
  const handleAddTask    = (data) => addTask(data)
  const handleToggleTask = (id, status) => updateTask(id, { status })
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-5 animate-fade-in">
      ...
      {tab === 'inbox' && (
        <div className="space-y-3">
          ...
          {inbox.map((item) => (
            <BrainCard key={item.id} item={item} onMove={handleMove} onDelete={deleteBrainItem} tasks={tasks} onAddTask={handleAddTask} onToggleTask={handleToggleTask} onDeleteTask={handleDeleteTask} />
          ))}
        </div>
      )}
          ...
          {backlog.length === 0 ? (
            ...
            backlog.map((item, i) => (
              <div key={item.id} className="relative">
                ...
                <BrainCard item={item} onMove={handleMove} onDelete={deleteBrainItem} tasks={tasks} onAddTask={handleAddTask} onToggleTask={handleToggleTask} onDeleteTask={handleDeleteTask} />
              </div>
            ))
          )}
        </div>
