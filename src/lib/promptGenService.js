...
export async function generatePersonalityHooks(apiKey, { ideia, publicoAlvo }, voiceOpts = {}) {
  const voiceCtx = buildVoiceContext(voiceOpts.brandVoice, voiceOpts.dislikedContent, voiceOpts.bannedWords, voiceOpts.posicionamento)
...
export async function generateHashtagRanking(apiKey, { palavrasChave }) {
  const system = withManualOperacional(
    `Você é especialista em SEO de Instagram. A partir de uma lista de palavras-chave de nicho, gera hashtags brasileiras segmentadas por volume de uso estimado e hashtags internacionais ("gringas") virais relacionadas.`
  )
...
export async function generateSalesCaption(apiKey, { publicoAlvo, objetivo, produto }, voiceOpts = {}) {
  ...
  const prompt = `Público-alvo: ${publicoAlvo}\nObjetivo principal: ${objetivo}${produto ? `\nProduto/serviço: ${produto}` : ''}\n\nResponda APENAS com um JSON no formato:\n{"legenda": "texto completo da legenda"}`
